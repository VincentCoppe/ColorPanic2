...
public class HoverBlock : MonoBehaviour
{
    ...
    public void DisplayCells(GridManager Manager, TileGameObject _prefab, HashSet<(int, int)> coords) {
        foreach((int, int) coord in coords) {
            if(!blocksHovered.ContainsKey(coord) && Manager.GridObject[coord.Item1, coord.Item2] == null) {
                GameObject go = Manager.Instantiate(_prefab.gameObject);
